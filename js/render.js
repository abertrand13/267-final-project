...
var leftTeapot = new Teapot( new THREE.Vector3( - 50, 0, - 100 ),
								$( "#vShaderMultiPhong" ).text(),
								$( "#fShaderMultiPhong" ).text() );

teapots.push( leftTeapot );

var centerTeapot = new Teapot( new THREE.Vector3( 0, 0, 0 ),
						    	$( "#vShaderMultiPhong" ).text(),
								$( "#fShaderMultiPhong" ).text() );
...
var rightTeapot = new Teapot( new THREE.Vector3( 50, 0, 100 ),
								$( "#vShaderMultiPhong" ).text(),
								$( "#fShaderMultiPhong" ).text() );
...
var standardRenderer =
	new StandardRenderer( webglRenderer, teapots, sc, dispParams );

var stereoUnwarpRenderer =
	new StereoUnwarpRenderer( webglRenderer, sc, dispParams );
...
function animate() {
	...
	if ( sc.state.renderingMode === STEREO_MODE ) {
		...
		standardRenderer.render(
			mat.modelMat, mat.stereoViewMat.L, mat.stereoProjectionMat.L );
		...
		standardRenderer.render(
			mat.modelMat, mat.stereoViewMat.R, mat.stereoProjectionMat.R );

		webglRenderer.setScissorTest( false );

	} else if ( sc.state.renderingMode === STEREO_UNWARP_MODE ) {
